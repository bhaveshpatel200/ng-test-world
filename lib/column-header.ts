import { Component, Input, Output, EventEmitter, ViewChild, SimpleChanges, ViewContainerRef } from '@angular/core';
            ...
            <tr>
                <th
                    *ngIf="all.hasCheckbox"
                    [class]="'bh-w-px'"
                    [ngClass]="{ 'bh-sticky bh-bg-[#f6f7fa] bh-z-[1]': all.stickyHeader || all.stickyFirstColumn, 'bh-top-0': all.stickyHeader, 'bh-left-0': all.stickyFirstColumn }"
                >
                    <div class="bh-checkbox">
                        <input #selectedAll type="checkbox" (click)="selectAll.emit(selectedAll.checked)" />
                        <div>
                ...
                <ng-container *ngFor="let col of all.columns; let j = index">
                    ...
                    >
                        <div class="bh-flex bh-items-center" [ngClass]="[col.headerClass ? col.headerClass : '']" (click)="all.sortable && col.sort && sortChange.emit(col.field)">
                            {{ col.title }}
                            <span *ngIf="all.sortable && col.sort" class="bh-ml-3 bh-sort bh-flex bh-items-center" [ngClass]="[currentSortColumn, currentSortDirection]">
                                <svg width="16" height="16" viewBox="0 0 14 14" fill="none">
                                    <polygon
                                        points="3.11,6.25 10.89,6.25 7,1.75"
                                        fill="currentColor"
                                        class="bh-text-black/20"
                                        [ngClass]="[currentSortColumn === col.field && currentSortDirection === 'asc' ? '!bh-text-primary' : '']"
                                    ></polygon>
                                    <polygon
                                        points="7,12.25 10.89,7.75 3.11,7.75"
                                        fill="currentColor"
                                        class="bh-text-black/20"
                                        [ngClass]="[currentSortColumn === col.field && currentSortDirection === 'desc' ? '!bh-text-primary' : '']"
                                    ></polygon>
                                </svg>
                            </span>
                        </div>
                        ...
                        <ng-container *ngIf="all.columnFilter && !isFooter">
                            <div *ngIf="col.filter" class="bh-filter bh-relative">
                                ...
                                <button *ngIf="col.type !== 'bool'" type="button" (click.stop)="toggleFilterMenu.emit(col)">
                                    <icon-filter class="bh-w-4"></icon-filter>
                                </button>

                                <column-filter [ngClass]="{ 'd-none': isOpenFilter !== col.field }" [column]="col" (close)="toggleFilterMenu.emit(null)" (filterChange)="filterChange.emit()"></column-filter>
                            </div>
                        </ng-container>
                    </th>
                </ng-container>
...
export class ColumnHeaderComponent {
    ...
    @Input() all: any;
    @Input() currentSortColumn: any;
    @Input() currentSortDirection: any;
    @Input() isOpenFilter: any;
    @Input() isFooter: any;
    @Input() checkAll: any;
    ...
    @Output('toggleFilterMenu') toggleFilterMenu: EventEmitter<any> = new EventEmitter<any>();
